...
def run():
    ...
    class MyWindow(QMainWindow):
        ...
        def initUI(self):
            ...
            self.slider.setGeometry(650, 120, 15, 300)
            self.slider.valueChanged[int].connect(self.change_transparency)
        ...
        def change_transparency(self, value):
            self.value = int((255 * value)  / 100)
            print(self.value)
            self.entrada = self.endereco2
            self.saida = 'images/imageTransparency.png'
            print(self.entrada[:-4])
            self.script = './mudarTransparencia.py'
            self.program = 'python ' + self.script + ' \"' + self.entrada + '\" ' + '\"' + self.saida + '\" ' + str(self.value)
            print(self.program)
            subprocess.run(self.program, shell=True)
            self.end_transform()
